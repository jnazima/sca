...
<?php
	 ...
	 // incluindo uma foto
	{
		...
		if ( !strpos($tipo_arquivo, "gif") and !strpos($tipo_arquivo, "jpeg") and !strpos($tipo_arquivo, "png") ) 
		{
		...
		$sql = " select cod_foto from fotos_noticia
		...
		{	
			...
			if( strpos($tipo_arquivo, "png") > 0 ) 
			{
				$extensao='png';
			}

			$sql = " insert into fotos_noticia (cod_noticia, extensao, descricao)
			...
			{			
				...
				$arquivo = "fotos_noticia/" . $cod_foto .'.'. $extensao;	
				...
				{ 	
					...
					mysql_query("delete from fotos_noticia where cod_foto = '$cod_foto' ", $conexao);
	...
	if( $_GET['acao'] == 'e' ) // excluindo uma foto
	{
		...
		$arquivo = "fotos_noticia/" . $_GET['cod_foto']  .'.'. $_GET['extensao'];
...
function excluir_foto(descricao, cod_foto, extensao)
{
	...
	{		
		...
		document.location = 'fotos_noticia.php?acao=e&cod_foto='+cod_foto+'&extensao='+extensao+'&cod_noticia=<?php echo $cod_noticia; ?>';	
...
<form action="fotos_noticia.php?acao=i&amp;cod_noticia=<?php echo $cod_noticia; ?>" onsubmit="return validar_foto();" method="post" enctype="multipart/form-data"name="form1" id="form1">
...
<?php
	...
	if( $rows > 0 )
	{
	  	...
	  	for($i=1;  $i<=$rows; $i++)
		{
			...
			$arquivo = "fotos_noticia/" . $dados['cod_foto']  .'.'. $dados['extensao'];	
