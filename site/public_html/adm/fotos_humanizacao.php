...
<?php
	...
	if( $rows_aux > 0 )
	{
	  ...
	  $nome = $aux['titulo'];
	  $cod_humanizacao = $aux['cod_humanizacao'];
	...
	echo '<script language="javascript"> 
							...
							document.location = "index.php?modulo=fotos_humanizacao";
					  </script>';
					  		exit; 
	}
	 ...
	 // incluindo uma foto
	{
		...
		$sql = " select cod_foto from fotos_humanizacao
				 where cod_humanizacao = '$cod_humanizacao' and descricao = '".$_POST['descricao']."'
			   ";
		...
		{	
			...
			$sql = " insert into fotos_humanizacao (cod_humanizacao, extensao, descricao)
					 values ( '" . 	($_GET['cod_humanizacao']) . "', 
					 		  '" . $extensao . "',
					 		  '" . 	($_POST['descricao']) . "')
					";
			...
			{			
				...
				$arquivo = "fotos_humanizacao/" . $cod_foto .'.'. $extensao;	
				...
				{ 	
					...
					mysql_query("delete from fotos_humanizacao where cod_foto = '$cod_foto' ", $conexao);
	...
	if( $_GET['acao'] == 'e' ) // excluindo uma foto
	{
		...
		$sql = " delete from fotos_humanizacao 
		...
		$arquivo = "fotos_humanizacao/" . $_GET['cod_foto']  .'.'. $_GET['extensao'];
...
function excluir_foto(descricao, cod_foto, extensao)
{
	...
	{		
		...
		document.location = 'fotos_humanizacao.php?acao=e&cod_foto='+cod_foto+'&extensao='+extensao+'&cod_humanizacao=<?php echo $cod_humanizacao; ?>';	
...
<p><strong><<< Cadastro de Fotos da Humaniza&ccedil;&atilde;o</strong></p>
<p><font color="#FF0000" size="1">Obs: Todas as fotos tem que ter Largura de 200 pixels e Altura de 150 pixels<br />S&oacute; ser&aacute; Permitido uma foto por not&iacute;cia.</font></p>
...
<form action="fotos_humanizacao.php?acao=i&amp;cod_humanizacao=<?php echo $cod_humanizacao; ?>" onsubmit="return validar_foto();" method="post" enctype="multipart/form-data"name="form1" id="form1">
...
<?php
	...
	if( $rows > 0 )
	{
	  	...
	  	for($i=1;  $i<=$rows; $i++)
		{
			...
			$arquivo = "fotos_humanizacao/" . $dados['cod_foto']  .'.'. $dados['extensao'];	
			echo '<img src="'.$arquivo.'" width="120" height="100"> <br>'; 
			echo $dados['descricao'];
			echo '</td> ';
			
			
			$link_excluir = '<a href="javascript:excluir(\''.$dados['descricao'].'\','.$dados['cod_humanizacao'].');"> Excluir </a> &nbsp;';//funcao excluir
