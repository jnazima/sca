...
<?php
	...
	// verificando o código do produto
   	...
   	$sql = " select  *
			 from medico
			 where cod_medico = '". $_GET['cod_medico'] . "' 
		   ";
	...
	echo '<script language="javascript"> 
							...
							document.location = "index.php?modulo=fotos_medicoss";
					  </script>';
					  		exit; 
	}
	 ...
	 // incluindo uma foto
	{
		...
		$sql = " select cod_foto from fotos_medicos
				 where cod_medico = '$cod_medico' and descricao = '".$_POST['descricao']."'
			   ";
		...
		{	
			...
			$sql = " insert into fotos_medicos (cod_medico, extensao, descricao)
					 values ( '" . 	($_GET['cod_medico']) . "', 
					 		  '" . $extensao . "',
					 		  '" . 	($_POST['descricao']) . "')
					";
			...
			{			
				...
				$arquivo = "fotos_medicos/" . $cod_foto .'.'. $extensao;	
				...
				{ 	
					...
					mysql_query("delete from fotos_medicos where cod_foto = '$cod_foto' ", $conexao);
	...
	if( $_GET['acao'] == 'e' ) // excluindo uma foto
	{
		...
		$arquivo = "fotos_medicos/" . $_GET['cod_foto']  .'.'. $_GET['extensao'];
...
function excluir_foto(descricao, cod_foto, extensao)
{
	...
	{		
		...
		document.location = 'fotos_medicos.php?acao=e&cod_foto='+cod_foto+'&extensao='+extensao+'&cod_medico=<?php echo $cod_medico; ?>';	
...
<p><b> <h2> <?php echo 'Nome do M&eacute;dico: ' . $nome; ?> </h2></b></p>
...
<form action="fotos_medicos.php?acao=i&amp;cod_medico=<?php echo $cod_medico; ?>" 
...
<?php
	...
	if( $rows > 0 )
	{
	  	...
	  	for($i=1;  $i<=$rows; $i++)
		{
			...
			$arquivo = "fotos_medicos/" . $dados['cod_foto']  .'.'. $dados['extensao'];	
			echo '<img src="'.$arquivo.'" width="120" height="100"> <br>'; 
			echo $dados['descricao'];
			echo '</td> ';
			
			
			$link_excluir = '<a href="javascript:excluir(\''.$dados['descricao'].'\','.$dados['cod_medico'].');"> Excluir </a> &nbsp;';//funcao excluir
