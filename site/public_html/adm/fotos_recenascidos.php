...
<?php
	...
	if( $rows_aux > 0 )
	{
	  ...
	  $nome = $aux['nome'];
	  $cod_rec_nascido = $aux['cod_rec_nascido'];
	...
	echo '<script language="javascript"> 
							...
							document.location = "index.php?modulo=fotos_recenascidos";
					  </script>';
					  		exit; 
	}
	 ...
	 // incluindo uma foto
	{
		...
		$sql = " select cod_foto from fotos_recenascidos
				 where cod_rec_nascido = '$cod_rec_nascido' and descricao = '".$_POST['descricao']."'
			   ";
		...
		{	
			...
			$sql = " insert into fotos_recenascidos (cod_rec_nascido, extensao, descricao)
					 values ( '" . 	($_GET['cod_rec_nascido']) . "', 
					 		  '" . $extensao . "',
					 		  '" . 	($_POST['descricao']) . "')
					";
			...
			{			
				...
				$arquivo = "fotos_recenascidos/" . $cod_foto .'.'. $extensao;	
				...
				{ 	
					...
					mysql_query("delete from fotos_recenascidos where cod_foto = '$cod_foto' ", $conexao);
	...
	if( $_GET['acao'] == 'e' ) // excluindo uma foto
	{
		...
		$sql = " delete from fotos_recenascidos 
		...
		$arquivo = "fotos_recenascidos/" . $_GET['cod_foto']  .'.'. $_GET['extensao'];
...
function excluir_foto(descricao, cod_foto, extensao)
{
	...
	{		
		...
		document.location = 'fotos_recenascidos.php?acao=e&cod_foto='+cod_foto+'&extensao='+extensao+'&cod_rec_nascido=<?php echo $cod_rec_nascido; ?>';	
...
<p><b> <h2> <?php echo 'Nome do Rec&eacute;m nascido: ' . $nome; ?> </h2></b></p>
<p><strong>CADASTRO DE FOTOS</strong></p>



<p>&nbsp;</p>

<b>Preencha os campos abaixo e clique em adicionar foto:</b>
<form action="fotos_recenascidos.php?acao=i&amp;cod_rec_nascido=<?php echo $cod_rec_nascido; ?>" 
	  onsubmit="return validar_foto();" 
	  method="post" 
...
<?php
	...
	if( $rows > 0 )
	{
	  	...
	  	for($i=1;  $i<=$rows; $i++)
		{
			...
			$arquivo = "fotos_recenascidos/" . $dados['cod_foto']  .'.'. $dados['extensao'];	
			echo '<img src="'.$arquivo.'" width="120" height="100"> <br>'; 
			echo $dados['descricao'];
			echo '</td> ';
			
			
			$link_excluir = '<a href="javascript:excluir(\''.$dados['descricao'].'\','.$dados['cod_rec_nascido'].');"> Excluir </a> &nbsp;';//funcao excluir
